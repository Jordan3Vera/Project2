...
export class OnDEmandPreloadingStrategy implements PreloadingStrategy {
  ...
  preload(route: Route, load: () => Observable<any>): Observable<any> {
    // Estamos escuchando a los valores de opciones de precarga emitidos por el nex() del servicio
    return this._preloadDemandOptions$.pipe(
      // Iteramos por cada valor recibido desde el servicio econ el next
